...
import { sanitizeRoomName, sanitizeRoomDescription, validateRoomName } from '@/lib/utils/sanitize';
...
export async function createRoomHandler(request: Request) {
    try {
        ...
        logInfo(`Create room request received:`, { name: data.name, action: 'create' });

        // Get total rooms created by user to generate room name
        const totalRoomsCreated = await prisma.room.count({
            where: {
                creatorId: session.user.id,
            },
        });

        // Auto-generate room name if empty or not provided
        // Format: Your_Room_1, Your_Room_2, etc.
        const isAutoGeneratedName = !data.name || data.name.trim().length === 0;

        if (isAutoGeneratedName) {
            data.name = `Your_Room_${totalRoomsCreated + 1}`;
            logInfo(`Auto-generated room name: ${data.name}`);
        } else {
            logInfo(`Custom room name provided: ${data.name}`);
            // Only validate custom names provided by user
            const nameValidation = validateRoomName(data.name);
            if (!nameValidation.valid) {
                throw new AppError(400, nameValidation.error || 'Invalid room name');
            }
            // Sanitize custom inputs
            data.name = sanitizeRoomName(data.name);
        }

        if (data.description) {
            data.description = sanitizeRoomDescription(data.description);
        }
